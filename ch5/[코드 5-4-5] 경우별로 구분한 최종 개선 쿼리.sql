-- tmp_t8_org에만 존재하는 레코드
SELECT
  'tmp_t8_org에만 존재'  AS source_table,
  org.*
FROM `tmp_t8_org`  org
LEFT JOIN `tmp_t8`  t ON
  org.`주차장관리번호`  = t.`주차장관리번호` AND
  org.`주차장명`  = t.`주차장명` AND
  org.`소재지도로명주소`  = t.`소재지도로명주소` AND
  org.`소재지지번주소`  = t.`소재지지번주소`
WHERE t.`주차장관리번호`  IS NULL
 
UNION ALL
 
-- tmp_t8에만 존재하는 레코드
SELECT
  'tmp_t8에만 존재'  AS source_table,
  t.*
FROM `tmp_t8`  t
LEFT JOIN `tmp_t8_org`  org ON
  t.`주차장관리번호`  = org.`주차장관리번호` AND
  t.`주차장명`  = org.`주차장명` AND
  t.`소재지도로명주소`  = org.`소재지도로명주소` AND
  t.`소재지지번주소`  = org.`소재지지번주소`
WHERE org.`주차장관리번호`  IS NULL
 
UNION ALL
 
-- 두 테이블에 모두 존재하고 변경된 레코드
SELECT
  CASE
